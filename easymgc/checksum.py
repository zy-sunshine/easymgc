...
import easymgc
from easymgc.const import PRELINK_BINARY,HASHING_BLOCKSIZE
from easymgc.localization import _
from easymgc import os
from easymgc import _encodings
from easymgc import _unicode_encode
import errno
import stat
import tempfile
...
def _open_file(filename):
	try:
		...
		if e.errno == errno.EPERM:
			raise easymgc.exception.OperationNotPermitted(func_call)
		elif e.errno == errno.EACCES:
			raise easymgc.exception.PermissionDenied(func_call)
...
if "WHIRLPOOL" not in hashfunc_map:
	...
	from easymgc.util.whirlpool import new as _new_whirlpool
...
if os.path.exists(PRELINK_BINARY):
	results = easymgc.subprocess_getstatusoutput(
...
def perform_multiple_checksums(filename, hashes=["MD5"], calc_prelink=0):
	...
	for x in hashes:
		if x not in hashfunc_map:
			raise easymgc.exception.DigestException(x+" hash function not available (needs dev-python/pycrypto or >=dev-lang/python-2.5)")
